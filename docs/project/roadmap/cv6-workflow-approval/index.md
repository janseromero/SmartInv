...
|------|------|----------------------|--------|
| CV6.E1 | Postgres Workflow Engine | `workflow.approvals` + `workflow.approval_events` tables driven by a typed state machine behind the `WorkflowEngine` interface | ⚪ Planned |
| CV6.E2 | Approval Queue UI | Planner / manager / finance see "my queue", can approve / request changes / reject with reason, see evidence strip and approval steps | ⚪ Planned |
| CV6.E3 | Audit Trail | Append-only `audit.events` topic + queryable view; every recommendation, approval, override, and source-write is logged | ⚪ Planned |
| CV6.E4 | Source-System Write Dispatch | Approved actions dispatched via Dramatiq workers to Maximo (and future sources) with idempotency and structured error handling | ⚪ Planned |
