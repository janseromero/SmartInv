...
| Code | Story | Status |
|------|-------|--------|
| CV6.E1.S1 | Implement `PostgresWorkflowEngine` behind the `WorkflowEngine` interface | 📥 Backlog |
| CV6.E1.S2 | `workflow.approvals` table + state machine: agent_proposed → planner_review → manager_review → finance_review → approved/rejected | 📥 Backlog |
| CV6.E1.S3 | `workflow.approval_events` append-only event log | 📥 Backlog |
| CV6.E1.S4 | Policy table: workflow type × value × criticality → required approvers | 📥 Backlog |
| CV6.E1.S5 | Idempotency keys on every state transition | 📥 Backlog |
| CV6.E1.S6 | Engine contract tests assert protocol compliance | 📥 Backlog |
