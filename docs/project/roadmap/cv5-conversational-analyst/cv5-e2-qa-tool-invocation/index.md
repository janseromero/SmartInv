...
**Status:** 🟢 In Progress — governed path + trace streaming delivered (S2 deferred)
**Depends on:** CV5.E1
...
| CV5.E2.S3 | `POST /agents/run` (JSON) **and `POST /agents/stream`** — SSE that streams the run *trace* (planning → tool calls → validating) then a terminal `answer` event with the validated envelope (never raw tokens, ADR-032) | ✅ Done |
