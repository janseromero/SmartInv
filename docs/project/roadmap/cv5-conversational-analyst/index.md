...
**Status:** 🟢 In Progress — governed backend path delivered (CV5.E1 + CV5.E2 core)
...
> **Governed backend path delivered (CV5.E1 + CV5.E2 core):** the whole governed path exists end to end — a `LiteLLMGateway` (SDK in-process), a versioned/scoped/audited **tool catalog** (`inventory.summary`, `risk.summary`), a linear orchestrator (plan → tools → compose → **grounding validate** → envelope) that **fails closed** on ungrounded numbers, and `POST /agents/run`. The **Ask SmartInv chat** is live (CV5.E2.S4/S5/S6): the `/analyst` page (with a conversation-history rail and suggested-prompts panel) plus a global topbar launcher (⌘K slide-over) usable from any screen. Answers render with the evidence strip, fail visibly when withheld, and **every turn is persisted** (`agent.conversations`/`runs`/`events`) so follow-ups continue the conversation and past ones reload. Answers **stream their governed run trace** (CV5.E2.S3): the chat shows live phases (planning → tool calls → validating) via `POST /agents/stream`, then the validated answer arrives whole — never raw tokens (ADR-032). CV5.E2 is now complete except the deferred PostgresSaver (S2). Still open in CV5: more tools (CV5.E1.S6), record-level source links (CV5.E3), and the eval suite (CV5.E4).
