...
> **Governed backend path delivered (CV5.E1 + CV5.E2 core):** the whole governed path exists end to end — a `LiteLLMGateway` (SDK in-process), a versioned/scoped/audited **tool catalog** (`inventory.summary`, `risk.summary`), a linear orchestrator (plan → tools → compose → **grounding validate** → envelope) that **fails closed** on ungrounded numbers, and `POST /agents/run`. The **Ask SmartInv chat** is live (CV5.E2.S4): the `/analyst` page plus a global topbar launcher (⌘K slide-over) usable from any screen, rendering grounded answers with the evidence strip and failing visibly when an answer is withheld. Still open: SSE trace streaming (CV5.E2.S3 — stream the *run trace*, deliver the validated answer whole, never raw tokens), more tools (CV5.E1.S6), record-level source links (CV5.E3), and the eval suite (CV5.E4). LangGraph/PostgresSaver deferred per ADR-032.
