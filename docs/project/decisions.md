...
- **Streaming means the run *trace*, not the answer tokens.** Fail-closed grounding runs on the *complete* answer, so streaming raw LLM tokens would put ungrounded numbers on screen before the validator can reject them — breaking the trust contract. SSE (CV5.E2.S3) will therefore stream governed progress events (planning, tool calls, validating) and deliver the *validated* answer as one terminal block. The CV5.E2.S4 chat UI is built on the JSON endpoint first and reuses that same render path.
