...
uv run python scripts/dedup.py [--tenant smartinv-dev]
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Recompute duplicate-detection candidates.")
    ...
    with tenant_session(str(tenant.id)) as session:
        summary = run_dedup(session, tenant.id)
